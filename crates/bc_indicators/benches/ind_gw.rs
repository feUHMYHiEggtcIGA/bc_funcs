...
fn ind_gw_rm_settings_rsi_empty_test_1(m: &mut Criterion) {
    let map_ind_t_rm_abstr_ = map_ind_t_rm_abstr();
    let map_args_ind_rm_ = map_args_ind(&SETTINGS_RSI_EMPTY);
    let map_map_args_mod_ = map_map_args_mod(&SETTINGS_RSI_EMPTY);
    let map_mod_all_abstr_ = map_mod_all_abstr();
    let map_func_rm_ind_abstr_ = map_func_rm_ind_abstr();
    let map_func_rm_mod_abstr_ = map_func_rm_mod_abstr();
    let map_func_window_ind_abstr_ = map_func_window_func_rm_abstr();
    ...
    m.bench_function(
        "ind_gw_rm_settings_rsi_empty_test_1", 
        |f| f.iter(||
            gw_ind_rm_abstr(
                &SRC_EL, 
                &SETTINGS_RSI_EMPTY, 
...
criterion_group!(benches, ind_gw_rm_settings_ind_test_1, ind_gw_rm_settings_rsi_empty_test_1);
